            ...
            if (_emailAnalysis != null) ...[
              ...
              Container(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Text(_emailAnalysis!, style: const TextStyle(fontSize: 13, height: 1.6, color: AppColors.textPrimaryLight)),
                ]),
              ),
            ],
          ]),
        ...
        if (_aiSummary != null) ...[
          ...
          Container(width: double.infinity, padding: const EdgeInsets.all(16),
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(height: 8),
              Text(_aiSummary!, style: const TextStyle(fontSize: 13, height: 1.5, color: AppColors.textPrimaryLight)),
            ]),
          ),
        ],
        const SizedBox(height: 14),
  ...
  Future<void> _analyzeEmail() async {
    ...
    try {
      ...
      final resp = await http.post(
        ...
        body: jsonEncode({'contents': [{'parts': [{'text': '''System: You are an expert career advisor and job search assistant. Your role is to analyze job-related emails and provide structured, actionable intelligence.
...
Be concise and specific. Use bullet points.'''}]}]}),
      ...
      if (resp.statusCode == 200) {
        ...
        setState(() => _emailAnalysis = data['candidates'][0]['content']['parts'][0]['text'] as String);
      } else {
  ...
  Future<void> _generateAISummary() async {
    ...
    try {
      ...
      final resp = await http.post(
        Uri.parse('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=$apiKey'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({'contents': [{'parts': [{'text': '''System: You are a job search strategist. Your role is to analyze a candidate's job email inbox and create an executive briefing.
...
Be specific with company names and dates.'''}]}]}),
      ...
      if (resp.statusCode == 200) {
        final data = jsonDecode(resp.body);
        setState(() => _aiSummary = data['candidates'][0]['content']['parts'][0]['text'] as String);
      } else {
