...
async def _call_groq(prompt: str, system: str = "", max_tokens: int = 4096, temperature: float = 0.7, json_mode: bool = True, model: str = None) -> str:
    """Helper to call Groq API with robust JSON mode."""
    target_model = model or GROQ_MODEL
    logger.info(f"[GROQ RESUME] Calling Groq with model: {target_model}, json_mode: {json_mode}")
    
    if not GROQ_API_KEY:
        logger.error("[GROQ RESUME] API key missing")
    ...
    if system:
        if json_mode:
            system += "\nIMPORTANT: Return ONLY raw JSON. No markdown formatting, no code blocks, no explanations. Just { or [."
        ...
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                if resp.status_code == 429 and attempt < max_retries:
                    ...
                    logger.warning(f"[GROQ RESUME] Rate limited (429), retrying in {wait}s (attempt {attempt + 1}/{max_retries})")
                    await asyncio.sleep(wait)
                    continue
                resp.raise_for_status()
                logger.info("[GROQ RESUME] Call successful")
                return resp.json()["choices"][0]["message"]["content"]
        except httpx.HTTPStatusError as e:
            logger.error(f"[GROQ RESUME] HTTP Error on attempt {attempt + 1}: {e.response.status_code} - {e.response.text}")
            if attempt == max_retries:
                raise
        except Exception as e:
            logger.error(f"[GROQ RESUME] Unexpected error on attempt {attempt + 1}: {e}")
            if attempt == max_retries:
                raise
