...
class PracticalProjectService {
  ...
  static Future<int> reviewGitHubRepo(
    ...
    final response = await http.post(
      Uri.parse('${AppConfig.baseUrl}/ai-tools/github/review'),
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode({
        'repoName': repo,
        'repoDescription': 'Submission for project: ${project.title}',
        'readmeContent': readmeContent,
        'fileTree': snippets.join('\n\n'),
      }),
    ).timeout(const Duration(seconds: 30));

    if (response.statusCode != 200) {
      throw StateError('Backend review failed: ${response.statusCode} - ${response.body}');
    }
