...
class _RoadmapTopicsPageState extends State<RoadmapTopicsPage>
  ...
  Future<void> _quizAndToggleTopic({
    RoadmapTopic? topic,
    RoadmapTopic? subtopic,
    int? topicIndex,
  }) async {
    final targetLabel = subtopic?.label ?? topic?.label ?? '';
    ...
    AssessmentDeck deck;
    try {
      deck = await AiFlashcardService.generateAssessmentDeck(
        roadmapName: widget.roadmap.name,
        skillOrRole: targetLabel,
        topics: [targetLabel],
        numQuestions: 5,
      );
    } catch (e) {
    ...
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => AssessmentViewPage(deck: deck),
      ),
    );
    ...
    if (result.accuracyPercent >= 50.0) {
      setState(() {
        if (subtopic != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Topic mastered! Awesome work.'),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'You scored ${result.accuracyPercent.toStringAsFixed(0)}%. '
            'You need at least 50% to pass. Review the topic and try again!',
          ),
  ...
  void _toggleTopic(RoadmapTopic topic, int topicIndex) {
    ...
    if (!markingDone) {
      setState(() => topic.isCompleted = false);
      _animateProgress();
      _fetchVideosForCurrentTopic();
      return;
    }

    _quizAndToggleTopic(topic: topic, topicIndex: topicIndex);
  }
